...
// Declare custom R3F elements so TypeScript doesn't complain
...
// 1×1 transparent pixel fallback so useTexture can be called unconditionally
const BLANK_PIXEL =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==';

// Front face = left half of the texture atlas; back face = right half
...
interface LanyardProps {
  ...
  lanyardWidth?: number;
  /** Controls the height of the canvas container. Defaults to '100vh'. */
  height?: string;
}
...
function Band({
  ...
  // Composite front/back images into the card's shared texture atlas
  const cardMap = useMemo(() => {
    const baseMap = materials.base.map as THREE.Texture;
    if (!frontImage && !backImage) return baseMap;

    const baseImg = baseMap.image as HTMLImageElement | HTMLCanvasElement;
    const W = (baseImg as HTMLImageElement).naturalWidth || (baseImg as HTMLCanvasElement).width;
    const H = (baseImg as HTMLImageElement).naturalHeight || (baseImg as HTMLCanvasElement).height;
